...
- `exported`: present validated `artifacts` and their MCP resource links. The runtime has already measured the finished file into `quality.export_qc` (duration, fps, resolution, audio span, `black_segments`) — read those numbers instead of probing the export yourself, and raise it with the user only when a field contradicts the request.
