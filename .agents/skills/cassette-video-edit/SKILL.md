...
- When the user no longer has that password, first tell them a replacement is emailed and the current one stops working **on every machine**, get their explicit agreement, then call `cassette_login` with `request_new_password=true` and `confirm_replace=true`. Never send this on a vague complaint like "I'm having password trouble" — ask first. It is rate-limited to a few attempts an hour and is destructive even when it reports a delivery failure, so never retry it to see if it works.
- `auth_invalid_password` does **not** prove the password is wrong — Cassette answers the same way for an address that has no Cassette access. Requesting a new password separates the two: it succeeds for a stale password and returns `auth_not_authorized` for an address without access. Do not tell the user they mistyped anything.
