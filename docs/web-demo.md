...
Cassette upload/analysis waits inherit `CASSETTE_JOB_TIMEOUT_SEC` by default. Set `CASSETTE_UPLOAD_TIMEOUT_SEC` only when you need a different upload timeout; `0` means wait forever.
